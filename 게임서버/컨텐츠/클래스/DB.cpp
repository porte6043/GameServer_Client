...
DB::DB(int threadcount)
{
	...
	*/
	...
	{
		HANDLE hThread = (HANDLE)_beginthreadex(nullptr, 0, DBThreadWrapping, this, 0, nullptr);
		if (hThread == 0)
		{
...
DB::~DB()
{
	...
	// 모든 스레드 종료 대기
	if (WAIT_FAILED == WaitForMultipleObjects(hThreads.size(), hThreads.data(), TRUE, INFINITE))
	{
		int err = GetLastError();
		LOG(L"Network_WanServer", CLog::LEVEL_ERROR, L"Failed to close db thread : ErrorCode : %d", err);
		// 이 코드는 null 포인터를 역참조하여 크래시를 발생시킵니다.
		int* Nint = NULL;
		*Nint = 0;
	}
...
void DB::DBThread()
{
		...
		while (1)
		{
			...
			case 1:
			{
				...
				if (mysql_query(&Mysql->conn, Querys[0].c_str()))
				{
					...
					LOG(L"DB", CLog::LEVEL_ERROR, L"failed mysql_query [error:%d]", errorno);
				}
				else
				{
					// mysql_query 성공
			...
			default:
			{
				...
				{
					LOG(L"DB", CLog::LEVEL_ERROR, L"failed transaction mysql_query [error:%d]", mysql_errno(&Mysql->conn));
					if (e == 1)
	...
	SetEvent(hEvent);
	std::cout << "db thread 종료" << std::endl;
	return;
}